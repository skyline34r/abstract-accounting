...
class StorageCore < ActiveSupport::TestCase
  test "share 2 to bank" do
    ...
    assert deals(:forex).state(nil).nil?, "Forex deal have state"
    assert_equal true,
    ...
    assert deals(:forex).state(nil).nil?, "Forex deal have state"
    assert deals(:bankaccount2).state(nil).nil?,
  ...
  private
  def check_state(aDeal, aDay, aAmount, aResource)
    deal_state = aDeal.state(aDay)
    if deal_state.nil?
      "#{aDeal.tag} state not found"
    elsif aAmount != deal_state.amount
      "#{aDeal.tag} state amount(#{deal_state.amount}) is not equal to #{aAmount}"
    elsif aResource != deal_state.resource
      "#{aDeal.tag} invalid state resource"
    else
      true
    end
  end
